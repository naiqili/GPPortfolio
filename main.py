...
trX0, trY0, teX0, teY0 = data1['rt_trainx'], data1['rt_trainy'], data1['rt_testx'], data1['rt_testy']
...
def dreshape(trX, trY, teX, teY, subn=100):
    return trX.reshape([ntrain, -1, lb])[:, :subn, :], trY.reshape([ntrain, -1, 1])[:, :subn, :], teX.reshape([ntest, -1, lb])[:, :subn, :], teY.reshape([ntest, -1, 1])[:, :subn, :]

subn=_subn
trX0, trY0, teX0, teY0 = dreshape(trX0, trY0, teX0, teY0, subn=subn)
nodes=subn

adj = np.eye(nodes).astype(int)
for i in args.matid:
    adj += all_mat[i][:subn, :subn]
...
np.savez('./tmp/original_res_phase%02d_iter%d_subn%d_lb%d'%(args.phase, args.maxiter, args.subn, args.lb), predY0=predY0, std_varY0=std_varY0, sample_Y0=sample_Y0, naive_rt=naive_rt, ucb_rt=ucb_rt)
