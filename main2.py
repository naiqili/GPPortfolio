...
parser.add_argument('--matid', type=int, default=4, nargs='*')
...
adj = np.eye(all_mat[0].shape[0]).astype(int)
for i in args.matid:
    adj += all_mat[i]
#     print(all_mat[i])
#     print(sum(all_mat[i]))
...
naive_rt = rt_v.copy()
# np.savez('./tmp2/res_phase%02d_iter%d_subn%d_lb%d_M%d_matcnt%d_inv%s_ggd%s'%(args.phase, args.maxiter, args.subn, args.lb, args.M, len(args.matid), args.inverse, args.ggd), predY0=predY0, std_varY0=std_varY0, val_predY0=val_predY0, val_std_varY0=val_std_varY0, val_sample_Y0=val_sample_Y0, sample_Y0=sample_Y0)
np.savez('./tmp2/%s_phase%02d_iter%d_subn%d_lb%d_M%d_matcnt%d_inv%s_ggd%s'%(args.dataset, args.phase, args.maxiter, args.subn, args.lb, args.M, len(args.matid), args.inverse, args.ggd), predY0=predY0, std_varY0=std_varY0, sample_Y0=sample_Y0)